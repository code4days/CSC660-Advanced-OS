	...
	public void run() {
		try {
			...
			while((inputLine = in.readLine()) != null) {
				...
				if (inputLine.split(":")[1].equals("EOR")) {

					out = new PrintWriter(nodeSockets.get(Integer.parseInt(inputLine.split(":")[0])).getOutputStream(), true);
					out.println("EOR");
				} else {
					...
					if (message.equals("FINISHED")) {
						...
						if (activeConnections == 0) {
							for (int i = 0; i < 7; i++) {
								...
								out.println("TERMINATE:" + -1); //sending terminate message to nodes
							}
						}
					} else {
						out = new PrintWriter(nodeSockets.get(destinationNode).getOutputStream(), true);
						out.println(message + ":" + sourceNode); //sending message to destination
						System.out.println(message + ":" + sourceNode); //sending message to destination
					}
				}
			}
			System.out.println("I'M OUT OF WHILE");
		}catch (IOException e) {
